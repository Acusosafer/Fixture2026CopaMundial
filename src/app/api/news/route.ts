...
// Fallback RSS bridges (no requieren API key de Twitter)
const EDUL_RSS_BRIDGES = [
  `https://rsshub.app/twitter/user/${EDUL_USERNAME}`,
  `https://nitter.privacydev.net/${EDUL_USERNAME}/rss`,
  `https://nitter.net/${EDUL_USERNAME}/rss`,
];

function parseEdulRssItems(xml: string): NewsItem[] {
  const itemRegex = /<item>([\s\S]*?)<\/item>/gi;
  const items: NewsItem[] = [];
  let match: RegExpExecArray | null;
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    const clean = text.replace(/<[^>]+>/g, '').replace(/https:\/\/t\.co\/\S+/g, '').trim();
    if (!clean) continue;

    const raw = `edul:${link}`;
    let hash = 0;
    for (let i = 0; i < raw.length; i++) hash = (Math.imul(31, hash) + raw.charCodeAt(i)) | 0;
    const id = `tweet-rss-${Math.abs(hash).toString(36)}`;

    items.push({
      id,
      title: clean,
      link: link.includes('x.com') || link.includes('twitter.com') || link.includes('nitter')
        ? link.replace(/nitter\.[^/]+/, 'x.com').replace('twitter.com', 'x.com')
        : link,
      pubDate,
      description: '',
...
async function fetchEdulFromRss(): Promise<NewsItem[]> {
  for (const url of EDUL_RSS_BRIDGES) {
    try {
      ...
      const items = parseEdulRssItems(xml);
...
async function fetchEdulTweets(bearerToken: string): Promise<NewsItem[]> {
  try {
    ...
    if (!userId) return fetchEdulFromRss();
    ...
    // Twitter API free tier returns 403 for timeline reads → fall back to RSS
    if (!res.ok) return fetchEdulFromRss();

    const json = await res.json() as { data?: Array<{ id: string; text: string; created_at: string }> };
    const tweets = json.data ?? [];

    if (tweets.length === 0) return fetchEdulFromRss();
...
async function fetchAllNews(): Promise<NewsItem[]> {
  ...
  const twitterPromise = bearerToken ? fetchEdulTweets(bearerToken) : fetchEdulFromRss();
